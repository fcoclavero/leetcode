...
class SparseVector:
    ...
    def dotProduct(self, vec: "SparseVector") -> int:
        non_zero_product_indices = set(self.non_zero_values.keys()) | set(
            vec.non_zero_values.keys()
        )
        total = 0
        for i in non_zero_product_indices:
            total += self.non_zero_values[i] * vec.non_zero_values[i]
        return total
